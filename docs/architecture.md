...
- braincrew-lab/langgraph-chat-ui의 frontend/를 ui/로 이식. 원본 docs/·README는
  ui/docs/에 보존 — **이식 시 upstream 문서를 빼먹지 말 것** (2026-07-17 교훈).
- **인증**: 로컬 dev는 standalone(무인증), Space 배포는 `AUTH_MODE=credentials`
  (NextAuth ID/PW) — 시드 계정만 활성. 계정은 기동 시 `ui/scripts/seed-users.mjs`
  가 Space 시크릿(`SEED_USER_EMAIL/PASSWORD`)으로 upsert (파일시스템 휘발 대응),
  자가 가입은 `REGISTRATION_POLICY=approval`로 pending 잠금(승인자 없음).
  passthrough(`/api`)는 세션 없으면 401 — 백엔드도 함께 잠긴다. bcryptjs는
  동적 import라 standalone 번들에 누락 → Dockerfile이 `cp -rL`로 보충.
...
- **HF iframe 함정**: Space는 huggingface.co 안의 cross-site iframe이라
  SameSite=Lax 쿠키가 요청에 실리지 않는다 — 선호 쿠키(연결·로케일)와
  NextAuth 세션·CSRF 쿠키 모두 production에서 `SameSite=None; Secure`
  (`crossSiteCookieAttributes`·`auth/config.ts cookies`). 브라우저가
  서드파티 쿠키를 차단하면 iframe 로그인은 실패 — 직접 URL로 안내.

### 모델 계층 — init_chat_model + config 라우팅 (벤더 5종)

- `config["configurable"]["model"]` 접두사 라우팅: `anthropic:`(기본
  claude-haiku-4-5, 프롬프트 캐싱) / `openai:` / `google_genai:` /
...
```
Agent_for_Newstep/             # (GitHub 저장소명 — 로컬 폴더는 구명 유지 가능)
