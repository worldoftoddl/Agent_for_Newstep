...
flowchart LR
    subgraph Space["HF Space (단일 컨테이너)"]
        UI["Next.js 채팅 UI<br/>:7860 (langgraph-chat-ui)"]
...
| LLM | 멀티 벤더 선택 — Claude(기본, 프롬프트 캐싱 실측 87% 히트) · GPT · Gemini · 오픈모델(HF Inference Providers: Qwen3.6·gpt-oss) · Ollama 로컬(qwen3:8b) |
...
| 파일 업로드 | Next.js `/api/workpapers` → 조서 폴더 저장, 채팅 첨부로 즉시 분석 (xlsx·xlsm·xls·docx, 20MB) |
| UI | Next.js 15 ([langgraph-chat-ui](https://github.com/braincrew-lab/langgraph-chat-ui) 벤더링, standalone 모드) — 스레드 사이드바·툴 호출 시각화·다크 테마 |
| 관측 | LangSmith 트레이싱 |
...
ui/                 채팅 UI (langgraph-chat-ui 벤더링 + 모델 선택·조서 업로드·브랜딩 이식)
